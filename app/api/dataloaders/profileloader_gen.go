...
type ProfileLoaderConfig struct {
	...
	Fetch func(keys []int64) ([]*models.Profile, []error)
...
type ProfileLoader struct {
	...
	cache map[int64]*models.Profile
...
func (l *ProfileLoader) Load(key int64) (*models.Profile, error) {
...
func (l *ProfileLoader) LoadThunk(key int64) func() (*models.Profile, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*models.Profile, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *ProfileLoader) LoadAll(keys []int64) ([]*models.Profile, []error) {
	results := make([]func() (*models.Profile, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	profiles := make([]*models.Profile, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ProfileLoader) LoadAllThunk(keys []int64) func() ([]*models.Profile, []error) {
	results := make([]func() (*models.Profile, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ProfileLoader) Prime(key int64, value *models.Profile) bool {
	l.mu.Lock()
	var found bool
...
func (l *ProfileLoader) unsafeSet(key int64, value *models.Profile) {
	if l.cache == nil {
		l.cache = map[int64]*models.Profile{}
	}
	l.cache[key] = value
}
